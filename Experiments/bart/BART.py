...
def bart(save_filename, num_runs):
    ...
    win = visual.Window(fullscr=False, units='pix', color=[200, 200, 200])
    scr_len = win.size[0]
    scr_hgt = win.size[1]
    x_mid = scr_len / 32
    y_mid = scr_hgt / 32
    ...
    #Control Experiment
    write_trigger('start_control', a)
    instructions_control = f"You have a gray balloon.\n\nPlease press SPACE to fill the balloon as many times as it takes for it to disappear from the screen." \
                           f"\n\n\nPress ENTER to begin"
    visual.TextStim(win, text=instructions_control, pos=(x_mid, y_mid), color=text_clr, height=text_dim).draw()
    win.flip()
    ...
    bottom_point = (x_mid, int(-0.25 * scr_hgt)) 
    for i in range(1, max_num_inflations + 1):
        ...
        draw_balloon(win, bottom_point, tri_dim, bln_size, control_balloon_clr)
        rwd_str = f"CUMULATIVE REWARD:\n${reward}"
        run_rwd = f"BALLOON REWARD: ${run_reward}"
        ...
        if 'space' in keys:
            ...
            #If explosion probability is met
            if i == explode_time:
                explosion = True
                run_reward = 0
                win.flip()
