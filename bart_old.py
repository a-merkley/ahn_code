...
def write_trigger(trig_type, a, arduino_plugin=True):
    switcher = {
...
def bart(save_filename, num_runs):
    ...
    write_trigger('start_experiment', a, arduino_plugin)
    ...
    for j in range(num_runs):
        write_trigger('start_run', a, arduino_plugin)
        run_reward = 0
        explosion = False
        cashin = False
        ...
        for i in range(1, max_num_inflations + 1):
            ...
            if 'space' in keys:
                write_trigger('button_press', a, arduino_plugin)
